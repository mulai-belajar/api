...
module.exports = {
  ...
  setLoggedIn: (body, condition) => {
    User.findOneAndUpdate(
      ...
      (error, resource) => {
        console.log(`User with id ${body.id} is logged out`)
      }
    )
  },
  ...
  isAuthenticated: (req, res, next) => {
    ...
    if (token !== undefined) {
      ...
      jwt.verify(token, process.env.JWT_SECRET, (error, decoded) => {
        ...
        // (7) Find the user based on the token _id/subject
        User.findById(decoded.sub, (error, user) => {
          ...
          if (error || !user) {
            ...
            // (9) The valid user is found!
            // That actual user is authenticated with valid token
            // console.log({ user })
            return next()
          }
        })
